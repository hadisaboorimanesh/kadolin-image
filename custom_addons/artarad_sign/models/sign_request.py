...
from odoo.addons.sign.models.sign_request import _fix_image_transparency
...
def get_reshaped_farsi_text(text):
    words = text.split()
    reshaped_words = []
    for word in words:
...
class artaradSignRequest(models.Model):
    ...
    def _generate_completed_document(self, password=""):
        ...
        if self.state != 'signed':
            raise exceptions.UserError(_("The completed document cannot be created because the sign request is not fully signed"))
                ...
                for item in items:
                    ...
                    if frame:
                        try:
                            ...
                            raise exceptions.ValidationError(_("There was an issue downloading your document. Please contact an administrator."))
                    ...
                    ########## Overrided ##########
                    if reshaper.has_arabic_letters(value):
                        custom_font = self.env["ir.config_parameter"].sudo().get_param("sign.use_custom_font_persian")
                        if not custom_font:
                            custom_font = "NotoSansArabic-Reg"
                        try:
                            path = os.path.dirname(os.path.realpath(__file__)) + "/../static/src/fonts/"
                            pdfmetrics.registerFont(TTFont(custom_font, path + custom_font + ".ttf"))
                            font = custom_font
                        except:
                            pass
                        value = get_reshaped_farsi_text(value)
                    ########## ######### ##########
        ...
        attachment_log = self.env['ir.attachment'].create({
            'name': "Certificate of completion - %s.pdf" % jdatetime.datetime.now().strftime('%Y-%m-%d - %H:%M:%S'),
            'raw': pdf_content,
            'type': 'binary',
            'res_model': self._name,
            'res_id': self.id,
