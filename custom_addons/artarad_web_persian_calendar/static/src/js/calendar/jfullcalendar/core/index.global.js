/*!
jFullCalendar Core v6.1.11
Docs & License: https://fullcalendar.io
(c) 2023 Adam Shaw
*/
var jFullCalendar = (function (exports) {
    'use strict';
    ...
    */
    ...
    /* jFullCalendar-specific DOM Utilities
    ----------------------------------------------------------------------------------------------------------------------*/
    ...
    */
    ...
    class NativeFormatter {
        ...
        format(date, context) {
            ...
            var ld = luxon.DateTime.fromJSDate(date.marker);
            var res = '\u202B';
            switch (this.severity) {
                case 1: // hour
                    res += ld.toFormat("HH:mm");
                    break;
                case 2:
                    res += ld.jday;
                    break;
                case 3: // week
                    res +=  this.extendedSettings.week == 'short' ? context.weekLabel + ' ' : '';
                    res += ld.jweekNumber;
                    break;
                case 5: // day
                    if (this.standardDateProps.day) {
                        res += ld.toFormat("jd");
                    }
                    if (this.standardDateProps.month) {
                        res += ' ' + ld.toFormat("jMM");
                    }
                    if (this.standardDateProps.year) {
                        res += ' ' + ld.toFormat("jyyyy");
                    }
            }
            return res + '\u202C';
            ////////// ///////// //////////
        ...
        formatRange(start, end, context, betterDefaultSeparator) {
            ...
            //     return full0;
            // }
            let ls = luxon.DateTime.fromJSDate(start.marker);
            let le = luxon.DateTime.fromJSDate(end.marker);
            if (ls.month == ls.month) {
                return ls.toFormat('jMMMM');
            }
            ////////// ///////// //////////
    ...
    */
    ...
    class DateProfileGenerator {
        ...
        buildRangeFromDuration(date, direction, duration, unit) {
            ...
            ////////// Overrided //////////
            if ((dateAlignment == 'month' || dateAlignment == 'year')) {
                var dm = luxon.DateTime.fromJSDate(date);
                if (dateAlignment == 'month') {
                    var ls = dm.startOf('jmonth');
                    var le = dm.endOf('jmonth').plus({day: 1});
                }
                else {
                    var ls = dm.startOf('jyear');
                    var le = dm.endOf('jyear').plus({day: 1});
                }
                res.start = new Date(Date.UTC(...[ls.year, ls.month - 1, ls.day]));
                res.end = new Date(Date.UTC(...[le.year, le.month - 1, le.day]));
            }
            ////////// ///////// //////////
            return res;
        }
    ...
    class DateEnv {
        ...
        startOf(m, unit) {
            ...
            if (unit === 'year') {
                ...
                let lm = luxon.DateTime.fromJSDate(m).startOf("jyear");
                return new Date(Date.UTC(...[lm.year, lm.month - 1, lm.day]));
            }
            else if (unit === 'month') {
                // return this.startOfMonth(m);
                let lm = luxon.DateTime.fromJSDate(m).startOf("jmonth");
                return new Date(Date.UTC(...[lm.year, lm.month - 1, lm.day]));
            }
            ////////// ///////// //////////
            if (unit === 'week') {
...
window.jFullCalendar = jFullCalendar;