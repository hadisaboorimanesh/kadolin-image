...
class ProductTabLine(models.Model):
    ...
    def check_tab_publish(self, tab):
        ...
        if (tab.tab_type == 'global' and tab.website_published == True) or (
                tab.tab_type == 'global' and user.has_group('website.group_website_designer')) or (
                tab.tab_type == 'specific product'):
            return True
        return False
    ...
    def write(self, vals):
        self.clear_caches()
        allowed_fields = ['sequence', 'tab_content', 'website_published', 'is_published', 'website_ids', 'tab_name', 'tab_type']
